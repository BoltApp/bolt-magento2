...
define([
    ...
    $.widget('bolt.boltLegacy', {
        options: {
            prototypeScript: /^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g,
            additionalInvalidateBoltCartJavascript: '',
            additionalJavascript: '',
            conditionScriptCallConfigureWithPromises: '',
            conditionScriptLoadJsDynamic: '',
            conditionScriptLoadJsConfig: '',
            additionalQuoteTotalsConditions: '',
        },
        ...
        _init: function () {
            ...
            if (!String.prototype.trim) {
                String.prototype.trim = function () {
                    return this.replace(self.options.prototypeScript, '');
                };
            }
            ///////////////////////
            ...
            require([
                ...
                var toggleCheckoutIfNeeded = function() {
                    ...
                    $.each(toggleCheckoutConfig.magentoButtons, function( index, buttonSelector ) {
                        ...
                        for (var i = 0; i < buttons.length; i++) {
                            ...
                            buttons[i].style.setProperty('display', boltRestricted ? 'inline-block' : 'none', 'important');
                        };
                    });
                };
                ////////////////////////////////////////////////////////
                ...
                var callbacks = {
                    ...
                    close: function () {
                        ...
                        if (callbacks.success_url) {
                            ...
                            // after the cart was changed from bolt checkout,
                            // i.e. the discount was applied
                            // added 0 delay to allow bolt connect finish its work
                            // before new Configure() call
                            setTimeout(invalidateBoltCart, 0);
                ...
                var invalidateBoltCartIfOutdated = function() {
                    ...
                    window.apiFlow = BoltState.boltCart.cartReference == "";
                    ...
                    eval(self.options.additionalInvalidateBoltCartJavascript);
                }
                    ...
                    if (!BoltState.initiateCheckout && !BoltState.magentoCart.isGuestCheckoutAllowed) {
                        if (BoltState.isUserLoggedIn === null) {
                            ...
                            require([
                                ...
                                BoltState.isUserLoggedIn = false;
                                if (customer && customer() && customer().firstname) {
                                    BoltState.isUserLoggedIn = true;
                                }
                                resolveCheckPromise();
                            });
                            return;
                ...
                var updateCartPaymentOnly = function() {
                    if (getCheckoutType() !== 'payment') {
                        return;
                    }
                ...
                var boltCheckoutSetup = function (el, dataChanged) {
                    ...
                    if (getCheckoutType() === 'payment') {
                        ...
                        window.magentoCartDataListener = function(data) {
                            BoltState.magentoCart = data;
                            if (boltCartDataID === undefined) {
                                var boltCart = customerData.get('boltcart')();
                                if (boltCart !== undefined
                                    && boltCart.data_id !== undefined
                                    && data.data_id<=boltCart.data_id
                                ) {
                                    // If we have bolt cart, haven't processed it before and timestamp is actual we need
                                    // process it now. We haven't done it before because wanted to check if bolt cart is present
                        ...
                        window.boltCartDataListener = function(data) {
                    ...
                    eval(self.options.additionalJavascript);
                ...
                $(document).on("ajax:addToCart", function () {
                    var callBoltAfterAddToCart = function() {
                        ...
                        eval(self.options.conditionScriptCallConfigureWithPromises);
                        // set flag to animate always present button when we catch cart updating
                        if (boltConfig.always_present_checkout) {
                            newItemAddedToCart = true;
                        }
                    }
                    eval(self.options.conditionScriptLoadJsDynamic);
                });
                ...
                eval(self.options.conditionScriptLoadJsConfig);
            });
                    ...
                    require([
                        ...
                        if (!jQuery.isEmptyObject(quote)) {
                            quote.totals.subscribe(function (totals) {
                                ...
                                if (expectCartRendering) {
                                    ...
                                    try {
                                        if ((totals.base_subtotal+totals.base_discount_amount) * 100 == BoltState.boltCart.total_amount.amount && eval(additionalQuoteTotalsConditions)) {
                                            return;
                                        }
                                    } catch(err) {}
                                }
                        ...
                        if (err.requireModules) {
                            ...
                            for (var key in err.requireModules) {
                                if (err.requireModules.hasOwnProperty(key) && err.requireModules[key]) {
                                    failedIds = failedIds ? failedIds + ', ' + err.requireModules[key] : err.requireModules[key];
                                }
                            }
