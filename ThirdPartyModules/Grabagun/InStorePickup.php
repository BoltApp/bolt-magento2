...
class InStorePickup
{
    ...
    const FFL_SHIPPING_ITEMS_ONLY = 'ffl';
    const NON_FFL_SHIPPING_ITEMS_ONLY = 'no_ffl';
    const MIXED_SHIPPING_ITEMS = 'mixed';
    const SHIPPING_RESTRICTION_TYPE_ATTRIBUTE_CODE = 'shipping_restriction_type';
    const SHIPPING_RESTRICTION_TYPE_SOT = 'SOT';
    const SALES_ORDER_ATTRIBUTE_FFL_SHIPPING_ADDRESS_DESCRIPTION = 'ffl_dealer_shipping_description';
    const FIREARMS_SHIPPING_CODE = 'firearmshipping';
    const PREFERRED_DEALER_ATTR_CODE = 'preferred_dealer_id';
     ...
     * @param \Bolt\Boltpay\Model\Api\Tax $boltTax
     */
     ...
     * @param $dealerRepository
     * @param $dealerFormatter
     * @param $grabgunShippingMethodHelper
     * @param $grabagunCollectionFactory
     * @param $quote
    ...
    public function getShipToStoreOptions(
        ...
        $dealerRepository,
        $dealerFormatter,
        $grabgunShippingMethodHelper,
        $grabagunCollectionFactory,
        $quote,
        $shippingOptions,
        $addressData
    )
    {
        try {
            ...
            if ($typeOfShipment == self::NON_FFL_SHIPPING_ITEMS_ONLY) {
                ...
                if ($this->customerSession->isLoggedIn()) {
                    $preferredStoreId = (int)$this->customerSession->getCustomer()->getData(self::PREFERRED_DEALER_ATTR_CODE);
                }
                ...
                if ($typeOfShipment == self::FFL_SHIPPING_ITEMS_ONLY) {

                    $result = [$shipToStoreOptions, []];
                } else if ($typeOfShipment == self::MIXED_SHIPPING_ITEMS) {
    ...
    public function getTypeOfShipment($grabgunShippingMethodHelper, $quote): string
    {
        ...
        if ($countFflItems > 0 && $countNonFflItems > 0) {
            $shippingType = self::MIXED_SHIPPING_ITEMS;
        } elseif ($countNonFflItems === 0) {
            $shippingType = self::FFL_SHIPPING_ITEMS_ONLY;
        } else {
            $shippingType = self::NON_FFL_SHIPPING_ITEMS_ONLY;
        }
    ...
    public function doesNfaItemExistInCart($quote)
    {
        foreach ($quote->getAllItems() as $item) {
            $productType = strtolower(
                $item->getProduct()->getAttributeText(self::SHIPPING_RESTRICTION_TYPE_ATTRIBUTE_CODE)
            );
            if ($productType === strtolower(self::SHIPPING_RESTRICTION_TYPE_SOT)) {
                return true;
            }
        }
        return false;
    ...
    /**
     * @param $result
     * @param $grabgunShippingMethodManagement
     * @param $grabgunShippingMethodHelper
     * @param $quote
    ...
    public function getShipToStoreCarrierMethodCodes(
        $result,
        $grabgunShippingMethodManagement,
        $grabgunShippingMethodHelper,
        $quote,
    ...
    )
    {
        ...
        $typeOfShipment = $this->getTypeOfShipment($grabgunShippingMethodHelper, $quote);
        if ($dealerStoreId && $typeOfShipment == self::FFL_SHIPPING_ITEMS_ONLY) {
    ...
    )
    {
        try {
            $typeOfShipment = $this->getTypeOfShipment($grabgunShippingMethodHelper, $quote);
            if ($typeOfShipment == self::FFL_SHIPPING_ITEMS_ONLY && isset($transaction->order->cart->in_store_shipments[0]->shipment)) {
    ...
    )
    {
        ...
        try {
            ...
            if ($typeOfShipment == self::MIXED_SHIPPING_ITEMS && isset($transaction->order->cart->in_store_shipments[0]->shipment)) {
                $shippingAddress = $quote->getShippingAddress();
                $shippingAddress->setCollectShippingRates(true);
                ...
                $quote->setData(self::SALES_ORDER_ATTRIBUTE_FFL_SHIPPING_ADDRESS_DESCRIPTION, $dealerShippingMethod);
                $quote->save();
            }
        } catch (\Exception $e) {
            $this->bugsnagHelper->notifyException($e);
    ...
    {
        $typeOfShipment = $this->getTypeOfShipment($grabgunShippingMethodHelper, $quote);
        if ($typeOfShipment == self::FFL_SHIPPING_ITEMS_ONLY && isset($transaction->order->cart->in_store_shipments[0]->address)) {
            ...
            foreach ($customFields as $customField) {
                if ($customField['external_id'] == 'set_selected_store_as_default' && $customField['value'] && isset($transaction->order->cart->in_store_shipments[0]->shipment->reference)) {
                    ...
                    if ($id) {
                        ...
                        $customer->setData(self::PREFERRED_DEALER_ATTR_CODE, $id);
                        $customer->save();
                    }
                }
            }
     ...
     * @throws \Exception
     */
    public function filterTaxResult($result, $shippingMethodHelper, $taxData, $totalsInformation, $currencyCode, $shipping_option, $ship_to_store_option, $quote)
    {
        $typeOfShipment = $this->getTypeOfShipment($shippingMethodHelper, $quote);
        if ($typeOfShipment == self::MIXED_SHIPPING_ITEMS) {
     ...
     * @param $addressData
     */
    public function setExtraAddressInformation($shippingMethodHelper, $addressInformation, $quote, $shipping_option, $ship_to_store_option, $addressData)
    {
        $typeOfShipment = $this->getTypeOfShipment($shippingMethodHelper, $quote);
        if ($typeOfShipment == self::MIXED_SHIPPING_ITEMS) {
            $dealerInfo = $this->getDealerInfoFromReference($ship_to_store_option['reference']);
        ...
        if (
            $typeOfShipment == self::FFL_SHIPPING_ITEMS_ONLY
    ...
    )
    {
        ...
        if ($typeOfShipment == self::MIXED_SHIPPING_ITEMS || $typeOfShipment == self::FFL_SHIPPING_ITEMS_ONLY) {
    ...
    public function beforeThrowingNoShippingMethodsException(
        $amastyShiprestrictionRuleCollection,
        $errors
    ){
        if ($errors) {
    ...
    )
    {
        if (
            ...
            && $this->getTypeOfShipment($shippingMethodHelper, $quote) == self::MIXED_SHIPPING_ITEMS
            && !empty($quote['ffl_dealer_shipping_description'])
        ) {

            $carrierData = explode('_', (string)$quote['ffl_dealer_shipping_description']);
            if ($carrierData[0] === self::FIREARMS_SHIPPING_CODE) {
     ...
     * @return mixed
     */
    public function filterShippingAddressForPaymentOnly($shipAddress, $cart)
    {
