...
class CreateOrder implements CreateOrderInterface
{
    ...
    public function createOrder($transaction, $immutableQuote)
    {
        ...
        if (! $createdOrder) {
            $this->eventsForThirdPartyModules->dispatchEvent("beforeValidateQuoteData", $quote, $transaction);
    ...
    public function validateShippingCost($quote, $transaction)
    {
        ...
        if ($quote->getShippingAddress()->getCreditAmount()
            || $this->eventsForThirdPartyModules->runFilter("skipValidateShippingCost", false, $quote, $transaction)) {
            return;
        }
